...
def mark_shadows(shadows: list[Shadow], state: MarketState, now: datetime) -> list[ShadowMark]:
    options = {o.symbol: o for o in state.options}
    marks = []
    for shadow in shadows:
        option = options.get(shadow.symbol)
        age = (now - shadow.timestamp).total_seconds()
        if not option or age < 60 or not 0 <= (now - option.quote_at).total_seconds() <= 120:
            continue
...
def score_agents(marks: list[ShadowMark]) -> list[AgentScore]:
    # At most one latest completed-horizon mark per shadow; repeated cycles cannot inflate samples.
    latest = {}
    for mark in sorted(marks, key=lambda m: m.timestamp):
        if mark.horizon_complete:
            latest[mark.shadow_id] = mark
    scores = []
    for agent in AGENTS:
        ...
        # Shrink toward neutral; cap counterfactual influence at +/-15 points.
        reliability = n / (n + 20)
        ...
        scores.append(AgentScore(
            ...
            basis="Neutral prior 50; completed 60-minute counterfactual outcomes only; no executed attribution",
        ))
    return scores
...
def review_positions(state: MarketState, regime: Regime, policy: Policy,
    ...
    for pos in state.positions:
        ...
        compatible = None if not option else not (
        ...
        elif option is None or hours is None or hours < policy.min_expiry_hours or not within_risk:
